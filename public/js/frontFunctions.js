...
showAlert: (message = "<b>Saved successfully!</b>", type = "success") => {
...
getAllPages: (order = 'date') => {
  return request({ url: `/api/page/order/${order}`, method: "GET"})
},
getToken: (email, password) => {
  return request({ url: "/api/auth", method: "POST", body: JSON.stringify({ email, password })})
},
